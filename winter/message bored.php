	...
	<div class="container">		
		<div class="all">
			...
			<form method="post" action="../php/message.php">
				<div>
					<textarea name="comment" id="content" placeholder="说点什么吧"></textarea>
				</div>
				<div class="setin">
				社交账号登录：
				<a href="##"><i class="iconfont">&#xe610;</i></a>
			...
			<?php   
			      if ($results->num_rows > 0) {
									    for(;$rows = $results->fetch_assoc();){
                                       	//for( ; $row['name'] != $rows['name'] && $rows['type'] != 2 && $idf != $rows['id']; ){$rows = $results->fetch_assoc();}
        								if( $row['name']==$rows['name'] && $rows['type'] == 2 ){
        									echo "留言人：".$rows['messagepeople']."<br>";//输出留言人
        									echo "留言  ：<br>".$rows["paasage"]."<br><br>";}//输出留言
        								 }}
			?>
			</div>
			<!-- <a href="#content">
